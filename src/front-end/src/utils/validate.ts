...
/** Determine whether the version number format is X.Y.Z */
...
/** Determine whether it is a second-generation ID card (18 digits) */
export const isChineseIdCard = (str: string) => {
  const reg = /^[1-9]\d{5}(?:18|19|20)\d{2}(?:0[1-9]|10|11|12)(?:0[1-9]|[1-2]\d|30|31)\d{3}[\dXx]$/
  return reg.test(str)
}

/** Determine whether it is Email (supports Chinese email) */
export const isEmail = (email: string) => {
  const reg = /^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/
  return reg.test(email)
}

/** Determine whether it is a MAC address */
...
/** Determine whether it is a license plate (compatible with new energy license plates) */
